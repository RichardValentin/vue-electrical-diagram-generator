<template>
    <div>
      <form @submit.prevent="handleSubmit">
        <h2>Cartouche Information</h2>
        <input type="text" name="projectName" placeholder="Project Name" v-model="cartouche.projectName" required />
        <input type="date" name="date" v-model="cartouche.date" required />
        <input type="text" name="reference" placeholder="Reference" v-model="cartouche.reference" required />
        <input type="text" name="client" placeholder="Client" v-model="cartouche.client" required />
        <input type="text" name="building" placeholder="Building" v-model="cartouche.building" required />
        <input type="text" name="folioNumber" placeholder="Folio Number" v-model="cartouche.folioNumber" required />
  
        <h2>Available Components</h2>
        <div>
          <button v-for="(comp, index) in availableComponents" :key="index" @click="selectComponent(comp)">
            {{ comp.name }}
          </button>
        </div>
        ...
        <div v-if="currentComponent">
          ...
          <input type="number" name="column" placeholder="Column" v-model.number="currentComponent.column" required />
          <input type="number" name="row" placeholder="Row" v-model.number="currentComponent.row" required />
          <button @click="addComponent">Add Component</button>
        </div>
  
        <h2>Selected Components</h2>
        <div>
          <div v-for="comp in selectedComponents" :key="comp.id">
            {{ comp.name }} - Type: {{ comp.type }}, Position: ({{ comp.column }}, {{ comp.row }})
            <div v-if="comp.children.length < 4">
              <button @click="selectComponent({ name: 'Add Child Component', type: 'child' }, comp.id)">
                Add Child Component
              </button>
            </div>
        ...
        <button type="submit" :disabled="!isFormValid">Generate Diagram</button>
  ...
  export default {
    data() {
      return {
        ...
        errorMessage: '',
        availableComponents: [
          { name: 'Disjoncteur 4P 25A 300mA', type: 'disjoncteur', characteristics: '4P 25A 300mA' },
          { name: 'Disjoncteur 4P 63A 30mA', type: 'disjoncteur', characteristics: '4P 63A 30mA' },
          { name: 'Prise 1P+N 10A', type: 'prise', characteristics: '1P+N 10A' },
          { name: 'Prise 1P+N 16A', type: 'prise', characteristics: '1P+N 16A' },
          { name: 'Groupe', type: 'composant', characteristics: 'Groupe' },
          { name: 'ECL', type: 'composant', characteristics: 'ECL' },
          { name: 'Reserve', type: 'composant', characteristics: 'Reserve' },
          { name: 'Chauffe eau', type: 'composant', characteristics: 'Chauffe eau' },
          { name: 'Split', type: 'composant', characteristics: 'Split' }
        ]
      };
    },
    mounted() {
