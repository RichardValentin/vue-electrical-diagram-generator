<template>
    <div :style="styles.container">
      <form @submit.prevent="handleSubmit" :style="styles.form">
        ...
        <h2 :style="styles.subHeading">Grid</h2>
  ...
  export default {
    ...
    return {
      ...
      availableComponents: [
        ...
        { name: 'Composant Générique', type: 'generic' }
      ],
    ...
    methods: {
      ...
      this.currentComponent = { ...component, characteristics: '', column: this.selectedColumn, row: this.selectedRow };
    },
    ...
    addComponent() {
      if (!this.currentComponent || !this.currentComponent.characteristics) {
        this.errorMessage = 'Veuillez spécifier les caractéristiques du composant.';
        return;
      }

      const newComponent = { ...this.currentComponent, id: Date.now(), children: [] };

      this.grid[this.selectedRow][this.selectedColumn] = newComponent;
    ...
    drawLink(svg, fromX, fromY, toX, toY) {
      ...
      drawComponentHierarchy(svg, component, x, y) {
        ...
        if (component.children && component.children.length > 0) {
          ...
          component.children.forEach((child, childIndex) => {
            const childX = childrenStartX + (childIndex * cellWidth);
      ...
      async handleSubmit() {
        ...
        this.selectedComponents.forEach((comp) => {
          const x = comp.column - 1; // Convertir en index de colonne (0-based)
          const y = comp.row - 1; // Convertir en index de ligne (0-based)
